...
- **Entités** : aucune entité **locale** (pas de modèle, pas de table persistent dans ce dépôt). Ressource distante consommée : un `transfer` avec les champs `id`, `from`, `to`, `price`, `seatsLeft` — inférés des propriétés lues dans `js/app.js`. `VÉRIFIÉ_CODE` pour l'usage de ces champs (lignes 19–22, 21) ; la forme réelle renvoyée par l'API est `INCONNU` pour les champs non affichés (horaires, compagnie, etc.).
...
- **Preuves** : `js/app.js` (export `reservations` ligne 6, `loadTransfers` lignes 8–39, `reserve` lignes 42–57, `cancelReservation` lignes 60–73, boucle rendant les boutons lignes 21–32), `index.html` (`<ul id="transfers-list">`, `<script src="js/app.js" type="module">`).
