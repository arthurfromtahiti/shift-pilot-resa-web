...
- **Entités** : aucune entité **locale** (pas de modèle, pas de table dans ce dépôt). Ressource distante consommée : un `transfer` avec les champs `from`, `to`, `price`, `seatsLeft` — inférés des propriétés lues dans `js/app.js` (`t.from`, `t.to`, `t.price`, `t.seatsLeft`). `VÉRIFIÉ_CODE` pour l'usage de ces champs ; la forme réelle renvoyée par l'API est `INCONNU` (aucun appel exécuté, aucun schéma d'API dans ce dépôt).
...
- **Indices de rattachement** : `transfers-list`, `loadTransfers`, `/transfers`, chemins `js/app.js` et `index.html`, champs `from`/`to`/`price`/`seatsLeft`, unité `XPF`.
